...
def test_boundary_splits_two_byte_char():
    ...
    assert _utf8_safe_boundary(data, 4) == 3


def test_boundary_splits_three_byte_char():
    ...
    assert _utf8_safe_boundary(data, 4) == 3
    assert _utf8_safe_boundary(data, 5) == 3


def test_boundary_splits_four_byte_char():
    """Boundary inside a 4-byte emoji backs up to before it."""
    # U+1F600 (😀) = 0xF0 0x9F 0x98 0x80
    data = b"aaa" + "\U0001f600".encode("utf-8")
    for split in (4, 5, 6):
        assert _utf8_safe_boundary(data, split) == 3


def test_boundary_at_char_start():
...
def test_save_exact_64kb_ending_with_emoji():
...
def test_save_64kb_plus_one_with_multibyte_at_boundary():
    ...
    assert len(encoded) == BUFFER_SIZE - 2 + 4 + 1  # 65539
    assert _roundtrip(content) == content


def test_save_70kb_mixed_ascii_and_emoji():
